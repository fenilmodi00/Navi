...
import { getGitHubRepoConfigs } from './github-sync.ts';
...
/**
 * Get all knowledge paths including GitHub repositories and default docs folder
 */
export function getAllKnowledgePaths(): string[] {
  const paths: string[] = [];
  
  // Add the default/configured knowledge path
  const defaultPath = getKnowledgePath();
  paths.push(defaultPath);
  
  // Add GitHub repository paths
  const githubConfigs = getGitHubRepoConfigs();
  for (const config of githubConfigs) {
    if (fs.existsSync(config.path)) {
      // If docsPath is specified, use that subdirectory, otherwise use the root
      const docsPath = config.docsPath 
        ? path.join(config.path, config.docsPath)
        : config.path;
      
      if (fs.existsSync(docsPath)) {
        paths.push(docsPath);
        logger.debug(`Added GitHub repo docs path: ${docsPath}`);
      } else {
        logger.warn(`GitHub repo docs path does not exist: ${docsPath}`);
      }
    } else {
      logger.warn(`GitHub repo path does not exist: ${config.path}`);
    }
  }
...
export async function loadDocsFromPath(
  ...
  const allPaths = getAllKnowledgePaths();
  
  let totalFiles = 0;
  let totalSuccessful = 0;
  let totalFailed = 0;
  
  for (const docsPath of allPaths) {
    if (!fs.existsSync(docsPath)) {
    ...
    if (files.length === 0) {
      logger.info(`No files found in path: ${docsPath}`);
      continue;
    }

    logger.info(`Found ${files.length} files to process in: ${docsPath}`);
    ...
    logger.info(
      `Document loading complete for ${docsPath}: ${successful} successful, ${failed} failed out of ${files.length} total`
    );
  ...
  logger.info(
    `Overall document loading complete: ${totalSuccessful} successful, ${totalFailed} failed out of ${totalFiles} total files from ${allPaths.length} paths`
  );
