...
export class KnowledgeService extends Service {
  ...
  constructor(runtime: IAgentRuntime, config?: Partial<KnowledgeConfig>) {
    ...
    // Helper function to get setting from runtime, config, or environment
    const getSetting = (key: keyof KnowledgeConfig, defaultValue?: any) => {
      return runtime.getSetting(key as string) || config?.[key] || process.env[key as string] || defaultValue;
    };

    this.config = {
      CTX_KNOWLEDGE_ENABLED: parseBooleanEnv(getSetting('CTX_KNOWLEDGE_ENABLED')),
      LOAD_DOCS_ON_STARTUP: parseBooleanEnv(getSetting('LOAD_DOCS_ON_STARTUP', 'true')),
      MAX_INPUT_TOKENS: getSetting('MAX_INPUT_TOKENS'),
      MAX_OUTPUT_TOKENS: getSetting('MAX_OUTPUT_TOKENS'),
    ...
    logger.debug(
      `KnowledgeService initialized for agent ${this.runtime.agentId}`
    );
  ...
  private async loadInitialDocuments(): Promise<void> {
    try {
      // Get repository configuration from runtime settings
      const repoConfig = {
        DOCS_REPO_1_URL: this.runtime.getSetting('DOCS_REPO_1_URL') || process.env.DOCS_REPO_1_URL,
        DOCS_REPO_1_PATH: this.runtime.getSetting('DOCS_REPO_1_PATH') || process.env.DOCS_REPO_1_PATH,
        DOCS_REPO_1_BRANCH: this.runtime.getSetting('DOCS_REPO_1_BRANCH') || process.env.DOCS_REPO_1_BRANCH,
        DOCS_REPO_1_DOCS_PATH: this.runtime.getSetting('DOCS_REPO_1_DOCS_PATH') || process.env.DOCS_REPO_1_DOCS_PATH,
        DOCS_REPOS: this.runtime.getSetting('DOCS_REPOS') || process.env.DOCS_REPOS,
      };
      
      const result: LoadResult = await loadDocsFromPath(this as any, this.runtime.agentId, undefined, repoConfig);
      if (result.successful > 0) {
        logger.info(
          `Loaded ${result.successful} documents from knowledge base for agent ${this.runtime.agentId}`
        );
      }
    } catch (error) {
      logger.error(
        `Error loading documents on startup for agent ${this.runtime.agentId}:`,
        error
      );
    }
  }
  ...
  static async start(runtime: IAgentRuntime): Promise<KnowledgeService> {
    ...
    if (service.runtime.character?.knowledge && service.runtime.character.knowledge.length > 0) {
      ...
      await service.processCharacterKnowledge(stringKnowledge).catch((err) => {
        logger.error(
          `Error processing character knowledge during startup: ${err.message}`,
          err
        );
      });
    }
