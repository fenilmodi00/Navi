...
export function getKnowledgeConfig(
  ...
  const loadDocsOnStartup = getSetting('LOAD_DOCS_ON_STARTUP');
  const hasRepoConfig = getSetting('DOCS_REPO_1_URL') || getSetting('DOCS_REPOS');
  
  // Force LOAD_DOCS_ON_STARTUP to true if repository configuration exists
  const shouldLoadDocs = loadDocsOnStartup === 'true' || (hasRepoConfig && loadDocsOnStartup !== 'false');
  
  return {
    CTX_KNOWLEDGE_ENABLED: getSetting('CTX_KNOWLEDGE_ENABLED') === 'true',
    LOAD_DOCS_ON_STARTUP: shouldLoadDocs,
    MAX_INPUT_TOKENS: getSetting('MAX_INPUT_TOKENS', '4000'),
    MAX_OUTPUT_TOKENS: getSetting('MAX_OUTPUT_TOKENS', '4096'),
